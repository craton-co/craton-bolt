...
- **CASE / CAST / COALESCE / NULLIF.** These **are** modeled in the AST (`Expr::Case`, `Expr::Cast`, `Expr::CastFormat`; COALESCE / NULLIF desugar to CASE) and lower to the GPU when the result dtype is numeric, `Bool`, `Date32`, or `Timestamp` (a fold of `selp.b32` / `selp.b64` for CASE — Date32/Timestamp ride along as bit-copies on their i32/i64 storage — and `cvt.*` for CAST). As of the 0.7 feature waves, **`Decimal128` CASE also lowers to the GPU** via the 128-bit predicated select (`Op::Select128`, a pair of `selp.b64`), and **CAST integer↔Decimal128 and (plain) Float↔Decimal128 lower to the GPU** (`Op::Div128` / `WidenToI128` / `NarrowI128ToInt` / `F64ToI128` / `I128ToF64`). A **`Utf8`-result CASE** is host-realized via `PhysicalPlan::StringProject` (`CaseUtf8`) over a bare scan, as is **`CAST … FORMAT`** (temporal↔string) and **`TRY_CAST` / `SAFE_CAST`** (NULL-on-failure host path; the safe path excludes Float↔Decimal128). See [`SQL_REFERENCE.md`](SQL_REFERENCE.md) §"CASE / CAST / COALESCE / NULLIF" for the per-dtype tier.
