...
| `metrics::Counter` (re-exported at crate root as `Counter`) | enum (`metrics`) | Counter identifiers (query counts, PTX cache hit/miss, launch counts, host fallbacks, PCIe byte totals). **Not** `#[non_exhaustive]` — adding variants is breaking until the attribute is added; enumeration order is the order `MetricsSnapshot::counters` yields. Experimental. |
| `metrics::Phase` (re-exported at crate root as `Phase`) | enum (`metrics`) | Per-query-phase identifier for the latency histograms (e.g. `Parse`). Same `#[non_exhaustive]` caveat as `Counter`. Experimental. |
...
| (M7 decision note) | — | The plan-cache and ptx-cache stats functions are the two strongest candidates for "promote to stable as a named-struct API in v0.7." All other experimental items (including the `exec::streaming::*` and `metrics::*` re-exports) should soak for one more release before promotion. |
