...
A proposal for how Javelin gets from 0.1.x (current) to a stable, semver-bound
...
3. **Documented limits.** Where Javelin doesn't fit a workload, the limit is
...
- `tracing` spans for every query phase: parse, plan, lower, codegen,
  ...
  config.
- Structured errors: `JavelinError::Parse { msg, span }`, similar for plan /
...
**Acceptance**
- Every error type in `JavelinError` documented with an example and the
  recovery path.
- The user guide walks a reader from `cargo add javelin` to a working
...
- Optional disk-backed PTX cache (`~/.cache/javelin/ptx/<hash>.cubin`),
...
- [ ] ClickBench result published; geometric mean within 2× of DuckDB on the
      queries we support; no Javelin query is > 10× slower than DuckDB on
      that suite.
...
Explicit exclusions, to be cited at the top of any "Javelin 1.0 release" post:
...
- **Not a CPU fallback engine.** If a query uses a feature Javelin doesn't
...
- Promote: power users can construct plans directly, build alternate frontends
  (e.g. Substrait → Javelin). Locks us into the current shape.
...
**Choice**: single `JavelinError` enum (current direction) vs typed errors per
...
**Recommendation**: single `JavelinError` with structured variants. Add
...
- **Error variants**: `JavelinError` is `#[non_exhaustive]` from 1.0; adding
...
- An issue filed against "Javelin produces wrong answer for X" is the
  exception, not the norm.
- A "Javelin vs DuckDB" benchmark from someone other than us appears, and we
  don't have to apologize for the result.
- A user's first 10 minutes with Javelin (registration → first query →
