...
- Both are positioned as alternatives to writing PTX-emitting Rust
  functions (Craton Bolt's pre-0.3 path) or NVRTC-driven C++ kernels.
...
1. **Does CUDA-Oxide 0.2 support PTX-text loading via the existing
   `CudaModule::from_ptx` path?** Craton Bolt's host-side loader is
...
- **DataFusion executor integration.** Craton Bolt as a DataFusion
  physical-plan executor: SQL parsing and logical planning happen in
  DataFusion, the executor offloads to Craton Bolt's PhysicalPlan/Kernel
  pipeline. This is the route to plugging Craton Bolt into a higher-level
  query engine (Ballista, etc.) without owning the SQL frontend.
  Scope-limited to the kernel shapes Craton Bolt already supports —
...
- **Persistent storage / catalog.** Craton Bolt remains a stateless
