...
- **Entry name:** dynamic, typically `bolt_predicate`. **PTX lines:** ~25 prologue + 3-8 per `Op` + 6 mask-store tail.
...
- **Entry name:** `bolt_reduce`. **PTX lines:** ~130 (53 fixed + 3 unrolled inter-warp strides + 5 unrolled warp-shuffle strides).
...
- **Entry names:** `bolt_groupby_keys`, `bolt_groupby_agg`. **PTX lines:** 103 (~45 + ~58).
...
- **Entry name:** `bolt_groupby_agg` (overloads #4 agg; host selects). **PTX lines:** ~59.
...
- **Entry names:** `bolt_groupby_keys_valid`, `bolt_groupby_agg_valid`. **PTX lines:** 187 across both.
...
- **Entry name:** `bolt_groupby_agg_valid` (overloads #6 agg). **PTX lines:** 105.
...
- **Entry names:** `bolt_prefix_scan`; `bolt_gather_{bool,i32,i64,f32,f64}`. **PTX lines:** 119.
...
- **Entry names:** `bolt_prefix_scan_u32`, `bolt_add_block_bases`. **PTX lines:** 94.
...
- **Entry name:** `bolt_groupby_shmem_sum_f64`. **PTX lines:** 93.
...
- **Entry name:** `bolt_groupby_shmem_multi_sum_f64_{n_vals}` (1..=4). **PTX lines:** ~70 scaffolding + per-aggregate unroll (~600 lines @ n_vals=4).
...
- **Entry name:** `bolt_groupby_shmem_count_u64`. **PTX lines:** 88.
...
- **Entry names:** `bolt_groupby_shmem_{min,max}_{i32,i64}` (4 variants). **PTX lines:** 92.
...
- **Entry name:** `bolt_partition`. **PTX lines:** 49.
...
- **Entry name:** `bolt_partition_i64`. **PTX lines:** 50.
...
- **Entry name:** `bolt_scatter`. **PTX lines:** 70.
...
- **Entry name:** `bolt_scatter_i64`. **PTX lines:** 69.
...
- **Entry name:** `bolt_partition_reduce`. **PTX lines:** 125.
...
- **Entry name:** `bolt_partition_reduce_i64`. **PTX lines:** 124.
...
- **Entry name:** `bolt_partition_reduce_multi_sum_{n_vals}` (1..=4). **PTX lines:** 117 scaffolding + N-unrolled body.
...
- **Entry name:** `bolt_partition_reduce_multi_sum_i64_{n_vals}` (1..=4). **PTX lines:** 118.
...
- **Entry name:** `bolt_partition_reduce_count`. **PTX lines:** 118.
...
- **Entry names:** `bolt_partition_reduce_{min,max}_{i32,i64}` (4 variants). **PTX lines:** 113.
...
- **Entry names:** `bolt_partition_reduce_{min,max}_{f32,f64}`. **PTX lines:** 120.
