# Rust-CUDA Toolchain Audit (Craton Bolt 0.3 milestone)
...
complementary to (not a replacement for) `cust_raw`. Craton Bolt already uses
...
- **Craton Bolt's CUDA 12.6 lands cleanly inside the supported window.**
...
**Avoiding nightly contamination of Craton Bolt proper.** The nightly pin is per
...
craton-bolt/
  ...
  src/                        # current Craton Bolt, stable
...
toolchain, so the parent crate can stay on stable. Craton Bolt's `src/jit/` PTX
...
CudaBuilder::new("kernels")        // path to the kernel crate
    ...
    .arch(NvvmArch::Compute70)     // sm_70 for Craton Bolt's target
    .build()
    .unwrap();
```
...
exactly like Craton Bolt's current hand-emitted PTX.
...
**Don't bet 0.3 on rust-cuda; bet 0.4.** The technical pieces Craton Bolt
