# 04 — Build-System Integration for Craton Bolt 0.3 (rust-cuda)

Status: design draft. Targets Craton Bolt's planned migration from runtime
...
Craton Bolt today is a single crate with a single `Cargo.toml`. rust-cuda
...
├── src/                    # existing Craton Bolt host code
├── kernels/
...
```rust
// build.rs at craton-bolt/build.rs
...
fn main() {
    ...
    CudaBuilder::new(manifest.join("kernels"))
        ...
        .arch(cuda_builder::NvvmArch::Compute75)  // tune per Craton Bolt support matrix
        .build()
        .unwrap();
}
```
...
the nvptx target flags. (a) is only worthwhile if Craton Bolt needs to
...
root pinning `nightly-2025-03-02` — drags every Craton Bolt contributor
...
Craton Bolt currently has a `cuda-stub` feature for docs.rs and for
...
(2021 → 2024). The 2025 reboot has a small maintainer pool. Craton Bolt
...
flowchart LR
    ...
    C["cuda_builder<br/>(in craton-bolt/build.rs)"]
    ...
    subgraph runtime["craton-bolt process"]
        F
        G
        H
    end
...
## Two concrete options for Craton Bolt 0.3
...
- **Cons:** binds Craton Bolt to a small upstream project. Every nightly
  bump in rust-cuda becomes a Craton Bolt chore. CUDA toolkit becomes a
...
- **Pros:** zero risk to existing users. Lets us learn the rust-cuda
  developer experience on real Craton Bolt kernels before committing.
