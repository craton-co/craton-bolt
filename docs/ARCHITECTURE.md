...
- **CTEs, subqueries, and window functions are supported** as of v0.7 — the parser accepts them and the planner lowers them (non-recursive CTEs inline at plan time; uncorrelated subqueries and window functions execute, with host-side evaluation where no GPU lowering exists yet). What remains out of scope is *correlated* subqueries and *recursive* CTEs. See [`SQL_REFERENCE.md`](SQL_REFERENCE.md) for the per-feature execution tier.
