...
- **CTEs, subqueries, and window functions are supported** — the parser accepts them and the planner lowers them (non-recursive CTEs inline at plan time; window functions execute, with host-side evaluation where no GPU lowering exists yet). Waves 14-17 closed the two items that were previously out of scope: *correlated* subqueries (including correlated `WHERE` subqueries, wave 14) and *recursive* CTEs now both execute. The recursive-CTE fixpoint is a host nested loop bounded by `CRATON_MAX_RECURSIVE_ITERATIONS` (default 1000); correlated `LATERAL` / apply execution is bounded by `CRATON_MAX_APPLY_ROWS` (default 100_000). See [`SQL_REFERENCE.md`](SQL_REFERENCE.md) for the per-feature execution tier and [`ENV_VARS.md`](ENV_VARS.md) for the caps.
