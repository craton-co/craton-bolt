...
fn lower_join_on(
    ...
    let mut out = Vec::new();
    collect_join_eq(e, &mut out, resolver, rhs_table)?;
    if out.is_empty() {
        return Err(BoltError::Sql(
...
fn collect_join_eq(
    ...
    match e {
        SqlExpr::Nested(inner) => collect_join_eq(inner, out, resolver, rhs_table),
        SqlExpr::BinaryOp {
            left,
            op: BinaryOperator::And,
            right,
        } => {
            collect_join_eq(left, out, resolver, rhs_table)?;
            collect_join_eq(right, out, resolver, rhs_table)
        }
        SqlExpr::BinaryOp {
            left,
