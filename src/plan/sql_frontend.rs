...
fn plan_select(select: &Select, provider: &dyn TableProvider) -> BoltResult<LogicalPlan> {
    ...
    if has_agg_in_select || !group_by_sql.is_empty() {
        ...
        if let Some(having_sql) = &select.having {
            let predicate = lower_expr_in_having(having_sql, &resolver, &having_agg_aliases)?;
...
/// aggregate with an alias — in which case the call resolves to the alias
/// (which is the only name the Project actually exposes). Everything else
/// delegates to `lower_expr`, which keeps the usual rules — bare columns
/// become column refs, non-aggregate function calls are still rejected,
/// etc.
///
/// `agg_aliases` maps `aggregate_output_name(agg)` to the SELECT alias it
/// was renamed to. Empty for HAVING with no SELECT aliases on aggregates;
/// in that case the lowerer falls back to the raw output name.
fn lower_expr_in_having(
    ...
    match e {
        SqlExpr::Nested(inner) => lower_expr_in_having(inner, resolver, agg_aliases),
        SqlExpr::BinaryOp { left, op, right } => {
            let lop = lower_binary_op(op)?;
            let l = lower_expr_in_having(left, resolver, agg_aliases)?;
            let r = lower_expr_in_having(right, resolver, agg_aliases)?;
            Ok(Expr::Binary {
                op: lop,
                left: Box::new(l),
                right: Box::new(r),
        ...
        SqlExpr::UnaryOp { op, expr } => match op {
            UnaryOperator::Plus => lower_expr_in_having(expr, resolver, agg_aliases),
            UnaryOperator::Minus => {
                ...
                let inner = lower_expr_in_having(expr, resolver, agg_aliases)?;
                Ok(Expr::Binary {
                    op: BinaryOp::Sub,
