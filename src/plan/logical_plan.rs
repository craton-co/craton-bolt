    ...
    pub fn dtype(&self, schema: &Schema) -> BoltResult<DataType> {
        match self {
            ...
            Expr::Binary { op, left, right } => {
                ...
                // type-checking the binary expression. This applies to
                // operators where the NULL is meaningful as a typed value
                // (arithmetic and comparison). For logical AND/OR we still
                // require both sides to type to `Bool`, which the NULL
                // peer-typing naturally satisfies: NULL opposite a Bool
                // typed expression becomes Bool. The runtime semantics of
                // NULL (e.g. `x = NULL` always returning NULL, not true)
                // are a separate concern handled by the executor; here we
                // just ensure the planner doesn't hard-error on the syntax.
