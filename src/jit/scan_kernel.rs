...
//!
//! ```text
//! .visible .entry javelin_predicate(
//!     .param .u64 javelin_predicate_param_0,        // input col 0
//!     ...
//!     .param .u64 javelin_predicate_param_{N-1},    // input col N-1
//!     .param .u64 javelin_predicate_param_{N},      // mask output (u8*)
//!     .param .u32 javelin_predicate_param_{N+1}_n_rows
//! )
//! ```
//!
...
impl RegAlloc {
    ...
    fn get(&self, reg: Reg) -> JavelinResult<&str> {
        self.mapping.get(&reg).map(|s| s.as_str()).ok_or_else(|| {
            JavelinError::Other(format!("scan_kernel: undefined register {:?}", reg))
        })
    }
...
impl PtxBuilder {
    ...
    fn emit(&mut self, line: &str) -> JavelinResult<()> {
        writeln!(self.body, "\t{}", line)
            .map_err(|e| JavelinError::Other(format!("scan_kernel: write failed: {}", e)))
    }
...
/// (`Utf8` in any form).
pub fn compile_predicate_kernel(spec: &KernelSpec, kernel_name: &str) -> JavelinResult<String> {
...
fn emit_op(
    ...
    match op {
        ...
        Op::Store { .. } => Err(JavelinError::Other(
...
fn byte_width(dtype: DataType) -> JavelinResult<usize> {
    dtype.byte_width().ok_or_else(|| {
        JavelinError::Other(format!("scan_kernel: variable-width dtype {:?}", dtype))
    })
}
...
fn write_signature(out: &mut String, b: &PtxBuilder, n_inputs: usize) -> JavelinResult<()> {
...
fn write_err(e: std::fmt::Error) -> JavelinError {
    JavelinError::Other(format!("scan_kernel: write failed: {}", e))
}
...
mod tests {
    ...
    fn requires_predicate() {
        ...
        spec.predicate = None;
        let err = compile_predicate_kernel(&spec, "javelin_predicate")
    ...
    fn region_eq_1_smoke() {
        ...
        let ptx = compile_predicate_kernel(&spec, "javelin_predicate")
        ...
        assert!(ptx.contains(".visible .entry javelin_predicate("));
        assert!(ptx.contains(".param .u64 javelin_predicate_param_0,"));
        assert!(ptx.contains(".param .u64 javelin_predicate_param_1,"));
        assert!(ptx.contains(".param .u32 javelin_predicate_param_2_n_rows"));
