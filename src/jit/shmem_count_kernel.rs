...
pub const KERNEL_ENTRY: &str = "bolt_groupby_shmem_count_u64";
...
///
/// ```text
/// .visible .entry bolt_groupby_shmem_count_u64(
...
pub fn compile_shmem_count_kernel() -> BoltResult<String> {
...
/// `std::fmt::Error` -> `BoltError`; matches `shmem_sum_kernel`.
fn write_err(e: std::fmt::Error) -> BoltError {
    BoltError::Other(format!("shmem_count_kernel: write failed: {}", e))
}
