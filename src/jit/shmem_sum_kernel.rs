...
//! ## Why this exists
//!
//! Craton Bolt's previous GROUP BY path issues one `atom.global.add` per input row
...
//! 100 cache lines — the GPU's atomic unit serialises hard and Craton Bolt runs
...
pub const KERNEL_ENTRY: &str = "bolt_groupby_shmem_sum_f64";
...
///
/// ```text
/// .visible .entry bolt_groupby_shmem_sum_f64(
...
pub fn compile_shmem_sum_kernel() -> BoltResult<String> {
...
fn write_err(e: std::fmt::Error) -> BoltError {
    BoltError::Other(format!("shmem_sum_kernel: write failed: {}", e))
}
