...
//!
//! 1. [`KEYS_KERNEL_ENTRY`] — `javelin_groupby_keys`. One thread per input row.
...
//!    table is touched.
//!
//! 2. [`AGG_KERNEL_ENTRY`] — `javelin_groupby_agg`. Re-runs the same hash +
...
//! Keys kernel:
//! ```text
//! .visible .entry javelin_groupby_keys(
...
pub const KEYS_KERNEL_ENTRY: &str = "javelin_groupby_keys";

/// Entry point of the aggregate-update kernel emitted by
/// [`compile_groupby_agg_kernel`].
pub const AGG_KERNEL_ENTRY: &str = "javelin_groupby_agg";
...
pub fn compile_groupby_keys_kernel() -> JavelinResult<String> {
...
fn write_err(e: std::fmt::Error) -> JavelinError {
    JavelinError::Other(format!("hash_kernels: write failed: {}", e))
}
