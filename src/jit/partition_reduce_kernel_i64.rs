...
pub fn compile_partition_reduce_kernel_i64() -> BoltResult<String> {
    ...
    // CAS-LOSER ACQUIRE: the ld.shared.s64 below is the key-read that races
    // against the publishing thread's `st.shared.u64 [block_keys+slot], key`
    ...
    // Volta+. TODO: switch to `ld.acquire.cta` when sm_60 is dropped.
    writeln!(
        ptx,
        "\t// CAS-LOSER ACQUIRE: see partition_reduce_kernel_i64.rs"
    )
    .map_err(write_err)?;
