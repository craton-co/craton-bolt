...
pub const KERNEL_ENTRY: &str = "javelin_partition_reduce_i64";
...
/// ```text
/// .visible .entry javelin_partition_reduce_i64(
...
/// block per partition.
pub fn compile_partition_reduce_kernel_i64() -> JavelinResult<String> {
...
fn write_err(e: std::fmt::Error) -> JavelinError {
    JavelinError::Other(format!("partition_reduce_kernel_i64: write failed: {}", e))
}
...
mod tests {
    ...
    fn has_correct_entry() {
        let ptx = compile_partition_reduce_kernel_i64().expect("compile");
        assert!(
            ptx.contains(".visible .entry javelin_partition_reduce_i64("),
