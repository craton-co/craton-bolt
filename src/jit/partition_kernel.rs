...
pub const KERNEL_ENTRY: &str = "javelin_partition";
...
/// `.visible .entry javelin_partition(.param .u64, .param .u64, .param .u64, .param .u32)`
...
/// Adapt a `std::fmt::Error` into a `JavelinError`. Mirrors the helper in
/// `shmem_sum_kernel.rs` — kept local so the two files stay independent.
fn write_err(e: std::fmt::Error) -> JavelinError {
    JavelinError::Other(format!("partition_kernel: write failed: {}", e))
}
...
mod tests {
    ...
    /// from co-loading this module with other Javelin kernels.
    #[test]
