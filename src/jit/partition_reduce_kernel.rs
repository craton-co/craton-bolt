...
pub const KERNEL_ENTRY: &str = "javelin_partition_reduce";
...
///
/// ```text
/// .visible .entry javelin_partition_reduce(
...
pub fn compile_partition_reduce_kernel() -> JavelinResult<String> {
...
/// Adapt a `std::fmt::Error` into a `JavelinError`. Same shape as the
/// helpers in sibling JIT files — kept local so each kernel emitter is
/// independent.
fn write_err(e: std::fmt::Error) -> JavelinError {
    JavelinError::Other(format!("partition_reduce_kernel: write failed: {}", e))
}
...
mod tests {
    ...
    /// from co-loading this kernel with other Javelin modules.
    #[test]
