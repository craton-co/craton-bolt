...
fn lookup_columns(batch: &RecordBatch, names: &[String]) -> JavelinResult<Vec<usize>> {
    let mut out: Vec<usize> = Vec::with_capacity(names.len());
    for n in names {
        let idx = batch.schema().index_of(n).map_err(|e| {
            JavelinError::Plan(format!("INNER JOIN: key column '{n}' not in batch: {e}"))
        })?;
        out.push(idx);
    }
    Ok(out)
...
fn column_name(e: &Expr, side: &str) -> JavelinResult<String> {
    match e {
...
fn empty_array_for_dtype(dt: &ArrowDataType) -> JavelinResult<ArrayRef> {
    Ok(match dt {
...
/// wrong row.
fn row_to_u32(row: usize) -> JavelinResult<u32> {
...
fn plan_dtype_to_arrow(d: crate::plan::logical_plan::DataType) -> JavelinResult<ArrowDataType> {
