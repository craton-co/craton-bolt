...
//!
//! Anything else returns `JavelinError::Other` with a `{:?}` of the
...
/// dtype is still NULL.)
fn eval_literal(lit: &Literal, n_rows: usize) -> JavelinResult<HostColumn> {
    Ok(match lit {
...
fn eval_arithmetic(
    ...
    match (lhs_u, rhs_u) {
        ...
        (other_l, other_r) => Err(JavelinError::Other(format!(
...
fn eval_comparison(
    ...
    match (lhs, rhs) {
        ...
        (l, r) => Err(JavelinError::Other(format!(
...
/// is a type error.
fn cast_column(col: HostColumn, to: DataType) -> JavelinResult<HostColumn> {
    if col.dtype() == to {
        return Ok(col);
    }
    Ok(match to {
...
fn cast_to_i32(col: HostColumn) -> JavelinResult<Vec<Option<i32>>> {
    Ok(match col {
...
fn cast_to_i64(col: HostColumn) -> JavelinResult<Vec<Option<i64>>> {
    Ok(match col {
...
fn cast_to_f32(col: HostColumn) -> JavelinResult<Vec<Option<f32>>> {
    Ok(match col {
        HostColumn::Bool(v) => v
            .into_iter()
...
fn cast_to_f64(col: HostColumn) -> JavelinResult<Vec<Option<f64>>> {
    Ok(match col {
        HostColumn::Bool(v) => v
            .into_iter()
            .map(|o| o.map(|b| if b { 1.0 } else { 0.0 }))
...
fn cast_to_bool(col: HostColumn) -> JavelinResult<Vec<Option<bool>>> {
    Ok(match col {
        HostColumn::Bool(v) => v,
...
fn cast_to_utf8(col: HostColumn) -> JavelinResult<Vec<Option<String>>> {
    match col {
        HostColumn::Utf8(v) => Ok(v),
        other => Err(JavelinError::Other(format!(
...
fn unify_numeric(a: DataType, b: DataType) -> JavelinResult<DataType> {
    ...
    match (a, b) {
        ...
        _ => Err(JavelinError::Other(format!(
