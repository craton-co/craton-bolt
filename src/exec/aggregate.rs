...
fn build_one_aggregate(
    ...
    match agg {
        AggregateExpr::Sum(expr) | AggregateExpr::Min(expr) | AggregateExpr::Max(expr) => {
            ...
            // Decimal128 SUM: a 128-bit integer reduction the primitive GPU
            // kernels don't cover. Route SUM through the dedicated i128
            // block-reduce kernel (`decimal_sum_from_batch`), which itself
            // falls back to a host fold when the GPU path declines. MIN/MAX
            // over Decimal128 are not yet supported and surface a clean type
            // error below via `reduce_column_from_batch`.
            if matches!(op, ReduceOp::Sum)
                && matches!(col_io.dtype, DataType::Decimal128(_, _))
            {
