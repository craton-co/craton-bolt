...
pub struct Engine {
    ...
    /// Mirrors the `device_idx` passed to [`CudaContext::new`]. Read by
    /// [`Engine::device`] for diagnostic introspection.
    device_idx: i32,
    /// v0.6 builder: soft cap on device-memory pool allocations in bytes.
    /// `None` means uncapped (the default — the existing pool's own
    /// `CRATON_BOLT_POOL_MAX_BYTES` env-var path stays in charge). Wired
    /// through from [`EngineBuilder::memory_budget`]; consulted by future
    /// pool integration but stored verbatim today so the API contract is
    /// observable via [`Engine::memory_budget_bytes`].
    memory_budget_bytes: Option<usize>,
    /// v0.6 builder: optional disk-backed PTX cache directory. `None`
    /// disables persistence (the default — the existing in-memory PTX
    /// cache in `jit::jit_compiler` is unaffected). Wired through from
    /// [`EngineBuilder::persistent_cache`]; readable via
    /// [`Engine::persistent_cache_path`].
    persistent_cache_path: Option<std::path::PathBuf>,
    /// v0.6 builder: whether [`EngineBuilder::enable_tracing`] was
    /// requested. Stored on the engine so it survives `Drop` introspection
    /// and so tests can observe the knob without re-reading env state.
