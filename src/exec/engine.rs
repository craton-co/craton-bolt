...
/// Cache key for [`Engine::module_cache`]: a content hash of the `KernelSpec`
/// plus the PTX entry-point name. The entry name distinguishes the two
/// different PTX shapes the projection path can emit for the same spec —
/// the full projection kernel ([`KERNEL_ENTRY`]) and the predicate-only
/// mask kernel ([`PREDICATE_ENTRY`]).
///
/// We do not derive `Hash` on `KernelSpec` directly because doing so would
/// require deriving `Hash` on every nested type in the planner IR (`Op`,
/// `Literal`, `Reg`, …) — a much broader change. Content-hashing via
/// `format!("{:?}", spec)` gives us a stable per-spec key for free: the
/// `Debug` derive is autogenerated and field-order deterministic, so two
/// specs with identical IR produce identical hash inputs. Collisions on the
/// 64-bit `DefaultHasher` are catastrophic only if a colliding pair would
/// also generate identical PTX modulo entry name; in practice they are
/// astronomically rare and bounded by the same risk surface as the
/// PTX-text-hash cache in `jit_compiler.rs`.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
struct ModuleCacheKey {
    /// Content hash of the `KernelSpec` (via `Debug` formatting).
    spec_hash: u64,
...
impl ModuleCacheKey {
    ...
    fn new(spec: &KernelSpec, entry: &'static str) -> Self {
        // `DefaultHasher` is `std::collections::hash_map::DefaultHasher`. We
        // feed it the `Debug` formatting of the spec — the autoderived impl
        // walks every field in declaration order and writes a stable string
        // form, which is exactly what we want for content hashing.
        let mut hasher = std::collections::hash_map::DefaultHasher::new();
        format!("{:?}", spec).hash(&mut hasher);
        Self {
            spec_hash: hasher.finish(),
            entry,
        }
    }
}
