...
///
/// `pub(crate)` (was module-private): the string method cluster split into
/// `engine_string_impl` launches kernels with this block size. Pure
/// visibility widening — no behaviour change.
...
pub(crate) fn materialize_generate_series_relation(
...
///
/// Pure (no GPU / engine state) so it is host-testable directly.
pub(crate) fn host_count_distinct_groupby(
    base: &RecordBatch,
    n_keys: usize,
...
pub(crate) fn host_multi_agg_groupby(
    base: &RecordBatch,
    n_keys: usize,
...
pub(crate) fn combine_scalar_aggregate_partials(
...
pub struct Engine {
    ...
    ///
    /// `pub(crate)` (was module-private): read by the engine-method clusters
    /// that were split out of this file into sibling modules
    /// (`engine_streaming_impl`, `engine_agg_impl`). Pure visibility widening —
    /// no behaviour change.
    pub(crate) tables: HashMap<String, Vec<RecordBatch>>,
...
impl Engine {
    ...
    /// flight.
    // `pub(crate)` (was module-private): called from the string method
    // cluster split out into `engine_string_impl`. Pure visibility widening.
    pub(crate) fn ensure_gpu_table(
        &self,
        name: &str,
    ...
    // `pub(crate)` (was module-private): called from the string method
    // cluster split out into `engine_string_impl`. Pure visibility widening.
    pub(crate) fn materialize_table(&self, name: &str) -> BoltResult<RecordBatch> {
    ...
    // `pub(crate)` (was module-private): called from the agg/query-source
    // method cluster split out into `engine_agg_impl`. Pure visibility widening.
    pub(crate) fn run_subplan(&self, plan: LogicalPlan) -> BoltResult<RecordBatch> {
        let plan = self.dict_registry.rewrite_plan(&plan)?;
    ...
    // ---- Host-side aggregate / row-source executors (split into
    // `engine_agg_impl.rs`) ------------------------------------------------
    //
    // `execute_count_distinct_groupby`, `execute_values_query`,
    // `execute_generate_series_query`, `execute_distinct_on`, and
    // `execute_multi_agg_groupby` were moved verbatim into the sibling module
    // `crate::exec::engine_agg_impl` to keep this file navigable. They remain
    // `impl Engine` methods dispatched from `Engine::sql` exactly as before —
    // only their visibility was widened to `pub(crate)`. No behaviour change.
    ...
    // ---- Streaming / morsel-driven execution (split into
    // `engine_streaming_impl.rs`) -----------------------------------------
    //
    // The `streamable_*` gate predicates and their `execute_streaming_*`
    // morsel drivers were moved verbatim into the sibling module
    // `crate::exec::engine_streaming_impl` to keep this file navigable. They
    // are still `impl Engine` methods and are dispatched from
    // [`Engine::execute`] below exactly as before — only their visibility was
    // widened to `pub(crate)` for the cross-module call. No behaviour change.
