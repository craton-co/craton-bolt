    ...
    let (pre, aggregate) = match plan {
        ...
        PhysicalPlan::Projection { .. } => {
            return Err(JavelinError::Other(
                "execute_groupby_valid: expected Aggregate plan, got Projection".into(),
            ))
        }
    };
...
) -> JavelinResult<(GpuVec<i64>, GpuVec<T>, GpuVec<u32>)> {
...
fn spill_lookup_err(key: i64) -> JavelinError {
    JavelinError::Other(format!(
