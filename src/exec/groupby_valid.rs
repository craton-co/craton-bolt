...
// (`download_pinned_{i32,i64,f32,f64}`), the key-packing types/functions
// (`KeyComponent`, `KeyValue`, `PackedKeys`, `pack_keys`, `decode_key`,
// `load_key_column_bits`, `and_masks`, `key_bit_width`, `canonicalise_f32`,
// `canonicalise_f64`) and the `unique_count` / `next_pow2` scan helpers were
// formerly copy-pasted here from `groupby.rs`. The V-17 `pack_keys`
// `wrapping_shl` hardening had to be applied to this copy SEPARATELY — exactly
// the drift-bug class the consolidation removes. They now live in the single
// canonical module `crate::exec::groupby_common`. Note: this module's old
...
// formerly copy-pasted here from `crate::exec::groupby` ("kept in lockstep"),
// which is exactly how the V-17 `pack_keys` `wrapping_shl` fix had to be
// applied twice. They are now the single canonical copies in
// `crate::exec::groupby_common` (imported at the top of this module). The old
// local `KeyComponent` had no `name` field; the canonical superset does, which
// is inert on this path (the sentinel-free executor never reads it). All call
// sites below — `pack_keys`, `decode_key`, `build_key_arrays_from_entries`,
// `unique_count`, `next_pow2` — are unchanged.
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    // dedup (groupby_common): the `spec` and `two_col_batch` fixtures plus the
    // `pack_keys` / `decode_key` / `and_masks` unit tests (incl. the
    // sentinel-colliding Float64 `-0.0` + NULL case and the V-17 round-trips)
    // were centralised into `crate::exec::groupby_common`'s `#[cfg(test)] mod
    // tests` when those helpers were consolidated there — coverage is the
    // union of this module's and `groupby.rs`'s former copies. The tests below
    // cover this executor's OWN (non-shared) host helpers.
