...
//! [`javelin_add_block_bases`](crate::jit::prefix_scan_multipass::ADD_BASES_KERNEL_ENTRY)
...
//!
//! The single-pass `javelin_prefix_scan` kernel reads a `u8` mask byte and
...
//! a sibling kernel
//! ([`javelin_prefix_scan_u32`](crate::jit::prefix_scan_multipass::SCAN_U32_KERNEL_ENTRY))
...
fn scan_block_sums(
    vals: GpuVec<u32>,
    stream: &CudaStream,
) -> JavelinResult<(GpuVec<u32>, usize)> {
...
///
/// 1. Run [`javelin_prefix_scan_u32`](SCAN_U32_KERNEL_ENTRY) on `vals` to
///    produce per-entry `local_indices` and per-block `block_sums`.
/// 2. Recursively turn that `block_sums` into `parent_bases` (+ total).
/// 3. Run [`javelin_add_block_bases`](ADD_BASES_KERNEL_ENTRY) to fold the
...
/// Launch the single-pass `javelin_prefix_scan` kernel over a u8 mask.
///
...
/// Launch the `javelin_prefix_scan_u32` kernel over a u32 input.
///
...
/// Launch the `javelin_add_block_bases` kernel.
///
